 ...
 */
export async function appendToSheet(range: string, values: string[][], recaptchaToken: string): Promise<{ success: boolean; error?: string; data?: unknown }> {
  try {
    if (!range || !Array.isArray(values) || values.length === 0 || !recaptchaToken) {
      throw new Error("Invalid input: range and values must be provided.");
    }

    const response = await axios.post("/api/sheets/append", { range, values, recaptchaToken });
